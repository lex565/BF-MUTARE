...
export type Product = { id:number; name:string; detail:string; price:number; icon:string; tint:string; description:string; category:'Fresh'|'Pantry'|'Drinks'|'Home' };
/**
 * Sample products, kept deliberately so testers have something to tap through
 * while the real shelves are still empty. The owner asked for these to stay.
 *
 * Everything else invented has gone: the fake identity, the fake order, the
 * fake saved addresses, the fake rider. Those were the ones that misled,
 * because a person cannot tell someone else's name or a stranger's delivery
 * from their own.
 */
export const PRODUCTS: Product[] = [
  { id:1, name:'Roller Meal', detail:'10 kg bag', price:8.5, icon:'🌽', tint:'#F4E6B7', category:'Pantry', description:'A dependable pantry staple for warm family meals, sourced for everyday value.' },
  { id:2, name:'Cooking Oil', detail:'2 litre bottle', price:4.2, icon:'🌻', tint:'#F6E0A8', category:'Pantry', description:'Versatile cooking oil for frying, baking and preparing everyday favourites.' },
  { id:3, name:'Fresh Tomatoes', detail:'1 kg', price:1.8, icon:'🍅', tint:'#F4D3CA', category:'Fresh', description:'Fresh, hand-selected tomatoes ready for relish, salads and family cooking.' },
  { id:4, name:'Mazoe Orange', detail:'2 litre bottle', price:3.6, icon:'🍊', tint:'#F3DDC0', category:'Drinks', description:'A refreshing Zimbabwean favourite with a bright, familiar orange flavour.' },
];
...
export function CustomerFlow({ route, setRoute, selectedId, setSelectedId, cart, setCart }: Props) {
  ...
  if (route === 'cart') return <Frame title="Your basket" back={back} cartCount={count}>
    {items.length === 0 ? <View style={s.empty}><Text style={s.emptyIcon}>🧺</Text><Text style={s.emptyTitle}>Your basket is waiting</Text><Text style={s.emptyText}>Add a few essentials and they will appear here.</Text><Pressable style={s.primary} onPress={() => setRoute(null)}><Text style={s.primaryText}>Continue shopping</Text></Pressable></View> : <>
      <View style={s.notice}><Text style={s.noticeText}>Freshly packed at Muroora Mart</Text></View>
  ...
  if (route === 'checkout') return <Frame title="Checkout" back={back} step="Step 2 of 2">
    <Section number="1" title="Buyer details"><Field label="Full name" value="" placeholder="Your full name" /><Field label="Email" value="" placeholder="you@example.com" keyboard="email-address" /><Field label="Phone" value="" placeholder="+263 77 000 0000" keyboard="phone-pad" /></Section>
    <Section number="2" title="Who is receiving this?">
      <Pressable style={s.toggleRow} onPress={() => setForMe(!forMe)}><View style={[s.checkbox,forMe&&s.checkboxOn]}>{forMe&&<Text style={s.check}>✓</Text>}</View><View><Text style={s.rowTitle}>I am the recipient</Text><Text style={s.rowSub}>Use my details for this delivery</Text></View></Pressable>
      {!forMe && <><Field label="Recipient name" value="" placeholder="Who receives it" /><Field label="Zimbabwe phone" value="" placeholder="+263 77 000 0000" keyboard="phone-pad" /><Field label="Delivery address" value="" placeholder="House number and street" /><Field label="Delivery notes" value="" placeholder="Landmarks, gate colour, who to ask for" /></>}
    </Section>
    <Section number="3" title="Delivery area"><Text style={s.help}>Demo zones—the real areas and fees will come from the backend later.</Text><Choice values={['Mutare Central','Dangamvura','Chikanga']} selected={zone} onSelect={setZone} /><View style={s.feeRow}><Text style={s.rowTitle}>Estimated delivery</Text><Text style={s.rowPrice}>${fee.toFixed(2)}</Text></View></Section>
    <Section number="4" title="Substitutions"><Choice values={['Contact me first','Similar product','No substitutions']} selected={substitution} onSelect={setSubstitution} /></Section>
    <Section number="5" title="Payment"><Choice values={['Pay on delivery','EcoCash (demo)','Card (demo)']} selected={payment} onSelect={setPayment} /><Text style={s.help}>Demo only. No payment will be processed.</Text></Section>
    <Summary subtotal={subtotal} fee={fee} />
    <Pressable style={s.primary} onPress={() => setRoute('confirmation')}><Text style={s.primaryText}>Place demo order · ${(subtotal+fee).toFixed(2)}</Text></Pressable>
    <Text style={s.disclaimer}>This prototype does not submit data or charge you.</Text>
  </Frame>;

  if (route === 'confirmation') return <Frame title="Order confirmed" back={back}>
    <View style={s.success}><View style={s.successIcon}><Text style={s.checkLarge}>✓</Text></View><Text style={s.successTitle}>The shopping is in motion.</Text><Text style={s.successText}>Your demo order has been received. We’ll keep you updated as it is prepared and delivered.</Text><View style={s.orderNumber}><Text style={s.orderLabel}>ORDER NUMBER</Text><Text style={s.orderValue}>MM-000124</Text></View></View>
